...
## Проверка содержания 18.09.2026

Следующие первичные источники использованы для точечных уточнений текста и преподавательских пояснений. Формулировки примеров и вопросы аудитории — авторские.

- [ISTQB CTFL 4.0.1, раздел 1.2.3](https://istqb.org/wp-content/uploads/2024/11/ISTQB_CTFL_Syllabus_v4.0.1.pdf): разграничение ошибочного действия, дефекта и отказа в терминологии тестирования.
- [RFC 9110, § 9.2.2](https://www.rfc-editor.org/rfc/rfc9110.html#section-9.2.2): идемпотентность относится к целевому эффекту, а не к совпадению ответов и журналов.
- [Prometheus: типы метрик](https://prometheus.io/docs/concepts/metric_types/): накопительный счётчик и измеритель текущего значения различаются; длины очереди в разные моменты нельзя суммировать как число уникальных задач.
- [Windows Firewall: правила](https://learn.microsoft.com/en-us/windows/security/operating-system-security/network-security/windows-firewall/rules): приоритет обычной явной блокировки над конфликтующим разрешением; порядок создания правил не определяет приоритет.
- [RFC 8446, § 1 и § 4.4](https://www.rfc-editor.org/rfc/rfc8446.html): защищённый обмен и аутентификация сторон; само наличие шифрования не заменяет проверку стороны соединения.
- [NIST: коллизия](https://csrc.nist.gov/glossary/term/collision): одинаковый хэш возможен для разных сообщений; совпадение хэшей не является абсолютным доказательством тождества.
- [BitLocker overview](https://learn.microsoft.com/en-us/windows/security/operating-system-security/data-protection/bitlocker/): границы защиты данных на носителе.
- [NIST SP 800-83, § 2.1](https://nvlpubs.nist.gov/nistpubs/legacy/sp/nistspecialpublication800-83.pdf): базовые различия вирусов, червей и троянов; исторический источник используется только для терминологии, не для современных настроек защиты.
- [EICAR: назначение тестового файла](https://www.eicar.org/download-anti-malware-testfile/): тест реакции антивируса без реального вредоносного кода. Файл не загружался и не запускался.

Порог длины пароля 14 символов — явно обозначенное условие учебного примера, не норматив и не рекомендация для всех сред. Расчёты MTBF, MTTR и готовности перепроверены по числам учебного журнала; вывод о готовности ограничен выбранным периодом и правилами учёта.
